...
# def get_weights_and_acts(name, cfg_dict, layer_id, outputs):
def get_weights_and_acts_410m(name, layer_id, outputs):
    device = "cuda" if torch.cuda.is_available() else "cpu"
...
def run_semantic_subspace_expms(fList_model_B, outputs):
    ...
    for layerID_2 in range(21, 22):
        print("Layer: " + str(layerID_2))
        dictscores = {}

        weight_matrix_np, reshaped_activations_A = get_weights_and_acts_410m(name, layerID_2, outputs)
        ...
        for concept_name in keywords.keys():
            ...
            for map_type in map_type_list:
                ...
                for metric_name, metric_fn in metric_dict.items():
                    ...
                    rand_corr_scores = score_rand_corr(2, weight_matrix_np, weight_matrix_2,
                    ...
                    rand_shuff_scores = shuffle_rand(2, X_subset, Y_subset, Y_subset.shape[0],
...
hookpoint = "layers." + str(layer_id_2) + ".mlp"
...
all_scores_dict = layer_to_dictscores[16]
...
all_scores_dict = layer_to_dictscores[21]
