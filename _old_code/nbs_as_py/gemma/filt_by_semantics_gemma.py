...
sae, cfg_dict, sparsity = SAE.from_pretrained(
    ...
    sae_id = f"blocks.{layerID}.hook_resid_post",
)
weight_matrix_np = sae.W_dec.cpu().detach().numpy()
...
for rankID in range(20):
    ...
    feat_B_lst = [ind_B for ind_B, ind_A in enumerate(highest_correlations_indices_AB) if ind_A == feat_ID]
    feature_idx_B = feat_B_lst[-1]
    print("FeatID: ", feat_ID, "| Count: ", sorted_feat_counts[rankID][1],
          "| Corr: ", highest_correlations_values_AB[feature_idx_B])
...
for feat_B, feat_A in enumerate(highest_correlations_indices_AB):
    ...
    if len(set(top_A_labels).intersection(set(top_B_labels))) > 0:
...
oneToOne_B = []
seen = set()
for ind_A, ind_B in zip(new_highest_correlations_indices_A, new_highest_correlations_indices_B):
    if ind_A in kept_modA_feats:
...
print("num feats after filt: ", len(oneToOne_A))
...
for ind_A, ind_B in zip(oneToOne_A, oneToOne_B):
    val = highest_correlations_values_AB[ind_B]
    if val > 0.1:
        new_oneToOne_A.append(ind_A)
        new_oneToOne_B.append(ind_B)
        new_highest_correlations_values.append(val)

# print("% unique after rmv 0s: ", num_unq_pairs / len(new_highest_correlations_indices_A))
# print("num feats after rmv 0s: ", len(new_highest_correlations_indices_A))
dictscores["num_feat_kept"] = len(new_oneToOne_A)
...
svcca(weight_matrix_np[new_oneToOne_A], weight_matrix_2[new_oneToOne_B], "nd")
