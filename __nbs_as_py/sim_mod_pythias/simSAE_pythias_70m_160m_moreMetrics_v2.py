...
def get_weights_and_acts(name, layer_id, outputs):
    ...
    # )
    ###########################################################################
    sae = Sae.load_from_hub(name, hookpoint=hookpoint)

    weight_matrix_np = sae.W_dec.cpu().detach().numpy()

    with torch.inference_mode():
        reshaped_activations_A = sae.pre_acts(outputs)
...
def run_expm(layer_id, outputs, outputs_2, layer_start, layer_end):
    ...
    for layerID_2 in range(layer_start, layer_end): # 0, 12
        ...
        weight_matrix_2, reshaped_activations_B = get_weights_and_acts(name, layerID_2, outputs_2)
...
with torch.inference_mode():
    ...
    outputs_2 = get_llm_actvs_batch(model_2, inputs, layer_id, batch_size=100, maxseqlen=300)
