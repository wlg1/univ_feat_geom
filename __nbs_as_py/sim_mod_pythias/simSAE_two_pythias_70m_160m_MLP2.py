...
layer_to_dictscores_1to1 = {3: {'mean_actv_corr': 0.7650287425578881,
...
def plot_svcca_byLayer(layer_to_dictscores):
    ...
    ax.bar_label(rects1, padding=10, fmt='%.4f')
    ax.bar_label(rects2, padding=10, fmt='%.4f')
...
def plot_js_byLayer(layer_to_dictscores):
    for key, sub_dict in layer_to_dictscores.items():
        for sub_key, value in sub_dict.items():
            sub_dict[sub_key] = round(value, 3)
    ...
    # Adding bar labels for better visualization
    ax.bar_label(rects1, padding=10, fmt='%.3f')
    ax.bar_label(rects2, padding=10, fmt='%.3f')
