...
name = "EleutherAI/sae-pythia-70m-32k"
weight_matrix_np, reshaped_activations_A = get_weights_and_acts(name, layerID, outputs)
...
name = "EleutherAI/sae-pythia-160m-32k"
weight_matrix_2, reshaped_activations_B = get_weights_and_acts(name, layer_id_2, outputs_2)
...
sorted_feat_counts = Counter(highest_correlations_indices_AB).most_common()
kept_modA_feats = [feat_ID for feat_ID, count in sorted_feat_counts if count <= max_num_matches]

filt_corr_ind_A = []
filt_corr_ind_B = []
seen_count = {} # feat : count
for ind_B, ind_A in enumerate(highest_correlations_indices_AB):
    if ind_A in kept_modA_feats:
        filt_corr_ind_A.append(ind_A)
        filt_corr_ind_B.append(ind_B)
    elif ind_A not in seen_count:  # only keep one if it's over count X
        seen_count[ind_A] = 0
        filt_corr_ind_A.append(ind_A)
        filt_corr_ind_B.append(ind_B)
    elif ind_A in seen_count:
        if seen_count[ind_A] < max_num_matches:
            filt_corr_ind_A.append(ind_A)
            filt_corr_ind_B.append(ind_B)
            seen_count[ind_A] += 1
# num_unq_pairs = len(list(set(filt_corr_ind_A)))
