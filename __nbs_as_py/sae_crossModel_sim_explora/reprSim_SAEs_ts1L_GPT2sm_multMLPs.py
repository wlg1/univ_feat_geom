...
filename = 'reprSim_SAEs_ts1L_GPT2sm_multMLPs' + '.csv'
...
def plot_scores(data):
    ...
    ax.set_title('TS-1L vs GPT2sm MLP Layer8 SAEs comparison')
    ax.set_xticks(x)
...
def plot_svcca_byLayer(layer_to_dictscores):
    ...
    # Adding labels, title and custom x-axis tick labels
    ax.set_ylabel('SVCCA')
    ax.set_title('TS-1L vs GPT2sm SAEs comparison by GPT2sm MLP Layers')
    ax.set_xticks(x)
...
fig, axes = plt.subplots(rows, cols, figsize=(12, 5))  # Adjust size as needed

for i, ax in enumerate(axes.flat):
    # Extracting paired and unpaired values for each metric
    paired_values = [data[i]['jaccard_paired'], data[i]['svcca_paired']]
    unpaired_values = [data[i]['jaccard_unpaired'], data[i]['svcca_unpaired']]
    ...
    ax.set_ylabel('Values')
    ax.set_title(f'Key {i}: Paired vs Unpaired')
    ax.set_xticks(x)
    ax.set_xticklabels(metrics)
    ax.set_ylim(0, 1)
    ax.legend()
