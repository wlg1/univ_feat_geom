...
file_path = '/content/drive/MyDrive/sae_files/ts_1L_21M_Wdec_df32768.pkl'  # Change the path if necessary
...
# ## load model and pretrained SAEs

# In[15]:


layer_name = "blocks.9.hook_mlp_out"
...
file_path = '/content/drive/MyDrive/sae_files/fActs_ts_1L_21M_anySamps__df-32768_v1.pkl'
with open(file_path, 'rb') as f:
...
feature_acts_model_A = feature_acts_model_A[:100, :, :]
...
# original_shape_A = feature_acts_model_A.shape
# original_shape_B = feature_acts_model_B.shape


# In[37]:


# original_activations_A = reshaped_activations_A.reshape(original_shape_A)
# original_activations_B = reshaped_activations_B.reshape(original_shape_B)
# (torch.equal(original_activations_A, feature_acts_model_A.cpu()))
...
# del original_activations_A
# del original_activations_B
torch.cuda.empty_cache()
...
# feature_acts_model_A = reshaped_activations_A.reshape(original_shape_A)
# feature_acts_model_B = reshaped_activations_B.reshape(original_shape_B)
...
import statistics
statistics.median(highest_correlations_values_AB)
...
jacc_paired = jaccard_similarity(weight_matrix_np, weight_matrix_2[highest_correlations_indices_AB])
jacc_paired
...
jacc_unpaired = jaccard_similarity(weight_matrix_np, weight_matrix_2)
jacc_unpaired
...
jacc_paired_1to1only = jaccard_similarity(weight_matrix_np[uniq_corr_indices_AB_forA], weight_matrix_2[uniq_corr_indices_AB_forB])
jacc_paired_1to1only
...
# unpaired
jacc_unpaired_1to1only = jaccard_similarity(weight_matrix_np[:len(uniq_corr_indices_AB_forA)], weight_matrix_2[:len(uniq_corr_indices_AB_forA)])
jacc_unpaired_1to1only
...
# num_feats = len(uniq_corr_indices_AB_forA)
# jacc_unpaired_1to1only = score_rand(num_feats, jaccard_similarity, shapereq_bool=False)
...
# k=4 due to smaller sample size
...
# jacc_actvs_paired = jaccard_similarity(reshaped_activations_A, reshaped_activations_B)
...
# random.shuffle(row_idxs)
# jacc_actvs_unpaired = jaccard_similarity(reshaped_activations_A[row_idxs, :], reshaped_activations_B)
...
jacc_actvs_paired = jaccard_similarity(reshaped_activations_A.t(), reshaped_activations_B.t()[highest_correlations_indices_AB])
jacc_actvs_paired
...
#unpaired
jacc_actvs_unpaired = jaccard_similarity(reshaped_activations_A.t(), reshaped_activations_B.t())
jacc_actvs_unpaired
...
jacc_actvs_paired_1to1only = jaccard_similarity(reshaped_activations_A.t()[uniq_corr_indices_AB_forA], reshaped_activations_B.t()[uniq_corr_indices_AB_forB])
jacc_actvs_paired_1to1only
...
# unpaired
jacc_actvs_unpaired_1to1only = jaccard_similarity(reshaped_activations_A.t()[:len(uniq_corr_indices_AB_forA)], reshaped_activations_B.t()[:len(uniq_corr_indices_AB_forA)])
jacc_actvs_unpaired_1to1only
...
svcca_paired = svcca(weight_matrix_np, weight_matrix_2[highest_correlations_indices_AB, :], "nd")
svcca_paired
...
svcca_unpaired = svcca(weight_matrix_np, weight_matrix_2, "nd")
svcca_unpaired
...
svcca_paired_1to1only = svcca(weight_matrix_np[uniq_corr_indices_AB_forA], weight_matrix_2[uniq_corr_indices_AB_forB], "nd")
svcca_paired_1to1only
...
# unpaired
svcca_unpaired_1to1only = svcca(weight_matrix_np[:len(uniq_corr_indices_AB_forA)], weight_matrix_2[:len(uniq_corr_indices_AB_forA)], "nd")
svcca_unpaired_1to1only
...
num_feats = len(uniq_corr_indices_AB_forA)
svcca_unpaired_1to1only = score_rand(num_feats, svcca, shapereq_bool=True)
...
svcca_paired_above0_9 = svcca(weight_matrix_np[(new_highest_correlations_indices_AB_A)], weight_matrix_2[new_highest_correlations_indices_AB, :], "nd")
svcca_paired_above0_9
...
# unpaired
svcca_unpaired_above0_9 = svcca(weight_matrix_np[:len(new_highest_correlations_indices_AB_A)], weight_matrix_2[:len(new_highest_correlations_indices_AB_A)], "nd")
...
# unpaired with after shuffling (same as above, just checking)
...
all_rand_scores = []
num_feats = len(new_highest_correlations_indices_AB)
for i in range(5):
    rand_modA_feats = np.random.randint(low=0, high=weight_matrix_np.shape[0], size=num_feats).tolist()
    rand_modB_feats = np.random.randint(low=0, high=weight_matrix_2.shape[0], size=num_feats).tolist()
...
print("Jaccard paired, 1-1 only: ", jacc_paired_1to1only)
print("Jaccard unpaired, 1-1 only: ", jacc_unpaired_1to1only)

print("\n")

print("Jaccard actvs paired: ", jacc_actvs_paired)
print("Jaccard actvs unpaired: ", jacc_actvs_unpaired)
print("Jaccard actvs paired, 1-1 only: ", jacc_actvs_paired_1to1only)
print("Jaccard actvs unpaired, 1-1 only: ", jacc_actvs_unpaired_1to1only)

print("\n")

print("SVCCA paired: ", svcca_paired)
print("SVCCA unpaired: ", svcca_unpaired)
print("SVCCA paired, 1-1 only: ", svcca_paired_1to1only)
print("SVCCA unpaired, 1-1 only: ", svcca_unpaired_1to1only)
